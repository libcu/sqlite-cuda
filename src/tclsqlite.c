...
*/
static SQLITE_METHOD int strlen30(const char *z){
  const char *z2 = z;
  while( *z2 ){ z2++; }
...
*/
static SQLITE_METHOD void closeIncrblobChannels(SqliteDb *pDb){
  IncrblobChannel *p;
...
*/
static SQLITE_METHOD int SQLITE_TCLAPI incrblobClose(
...
*/
static SQLITE_METHOD int SQLITE_TCLAPI incrblobInput(
  ClientData instanceData,
  char *buf,
  int bufSize,
  int *errorCodePtr
...
*/
static SQLITE_METHOD int SQLITE_TCLAPI incrblobOutput(
  ClientData instanceData,
  CONST char *buf,
  int toWrite,
  int *errorCodePtr
...
*/
static SQLITE_METHOD int SQLITE_TCLAPI incrblobSeek(
  ClientData instanceData,
  long offset,
  int seekMode,
  int *errorCodePtr
...
static SQLITE_METHOD void SQLITE_TCLAPI incrblobWatch(
  ClientData instanceData,
  int mode
){
  /* NO-OP */
}
static SQLITE_METHOD int SQLITE_TCLAPI incrblobHandle(
  ClientData instanceData,
  int dir,
  ClientData *hPtr
){
...
static SQLITE_CONSTANT Tcl_ChannelType IncrblobChannelType = {
  "incrblob",                        /* typeName                             */
...
*/
static SQLITE_METHOD int createIncrblobChannel(
  Tcl_Interp *interp,
  SqliteDb *pDb,
  const char *zDb,
  const char *zTable,
...
*/
static SQLITE_METHOD int safeToUseEvalObjv(Tcl_Interp *interp, Tcl_Obj *pCmd){
...
** structure.
*/
static SQLITE_METHOD SqlFunc *findSqlFunc(SqliteDb *pDb, const char *zName){
  SqlFunc *p, *pNew;
...
*/
static SQLITE_METHOD void dbFreeStmt(SqlPreparedStmt *pStmt){
#ifdef SQLITE_TEST
...
*/
static SQLITE_METHOD void flushStmtCache(SqliteDb *pDb){
...
** deleted.
*/
static SQLITE_METHOD void SQLITE_TCLAPI DbDeleteCmd(void *db){
...
** to execute SQL.
*/
static SQLITE_METHOD int DbBusyHandler(void *cd, int nTries){
...
*/
static SQLITE_METHOD int DbProgressHandler(void *cd){
  SqliteDb *pDb = (SqliteDb*)cd;
  int rc;
...
*/
static SQLITE_METHOD void DbTraceHandler(void *cd, const char *zSql){
  SqliteDb *pDb = (SqliteDb*)cd;
  Tcl_DString str;
...
*/
static SQLITE_METHOD int DbTraceV2Handler(
...
*/
static SQLITE_METHOD void DbProfileHandler(void *cd, const char *zSql, sqlite_uint64 tm){
  SqliteDb *pDb = (SqliteDb*)cd;
  Tcl_DString str;
  char zTm[100];
...
** of being committed.
*/
static SQLITE_METHOD int DbCommitHandler(void *cd){
  SqliteDb *pDb = (SqliteDb*)cd;
  int rc;
...
static SQLITE_METHOD void DbRollbackHandler(void *clientData){
...
*/
static SQLITE_METHOD int DbWalHandler(
  void *clientData,
  sqlite3 *db,
  const char *zDb,
  int nEntry
...
static SQLITE_METHOD void setTestUnlockNotifyVars(Tcl_Interp *interp, int iArg, int nArg){
  char zBuf[64];
...
static SQLITE_METHOD void DbUnlockNotify(void **apArg, int nArg){
  int i;
  for(i=0; i<nArg; i++){
...
*/
static SQLITE_METHOD void DbPreUpdateHandler(
  void *p,
  sqlite3 *db,
  int op,
  const char *zDb,
...
static SQLITE_METHOD void DbUpdateHandler(
  void *p,
  int op,
  const char *zDb,
  const char *zTbl,
...
static SQLITE_METHOD void tclCollateNeeded(
  void *pCtx,
  sqlite3 *db,
  int enc,
  const char *zName
...
** using TCL script.
*/
static SQLITE_METHOD int tclSqlCollate(
  void *pCtx,
  int nA,
  const void *zA,
  int nB,
...
** using TCL script.
*/
static SQLITE_METHOD void tclSqlFunc(sqlite3_context *context, int argc, sqlite3_value**argv){
...
*/
static SQLITE_METHOD int auth_callback(
  void *pArg,
  int code,
  const char *zArg1,
  const char *zArg2,
...
*/
static SQLITE_METHOD char *local_getline(char *zPrompt, FILE *in){
  char *zLine;
  int nLine;
  int n;
...
*/
static SQLITE_METHOD int SQLITE_TCLAPI DbTransPostCmd(
...
*/
static SQLITE_METHOD int dbPrepare(
...
*/
static SQLITE_METHOD int dbPrepareAndBind(
  SqliteDb *pDb,                  /* Database object */
...
*/
static SQLITE_METHOD void dbReleaseStmt(
...
*/
static SQLITE_METHOD void dbReleaseColumnNames(DbEvalContext *p){
  if( p->apColName ){
    int i;
    for(i=0; i<p->nCol; i++){
...
**     set ${pArray}(*) {a b c}
*/
static SQLITE_METHOD void dbEvalInit(
...
*/
static SQLITE_METHOD void dbEvalRowInfo(
...
*/
static SQLITE_METHOD int dbEvalStep(DbEvalContext *p){
...
*/
static SQLITE_METHOD void dbEvalFinalize(DbEvalContext *p){
  if( p->pPreStmt ){
...
** the DbEvalContext structure passed as the first argument.
*/
static SQLITE_METHOD Tcl_Obj *dbEvalColumnValue(DbEvalContext *p, int iCol){
  sqlite3_stmt *pStmt = p->pPreStmt->pStmt;
...
static SQLITE_METHOD int DbUseNre(void){
  int major, minor;
...
*/
static SQLITE_METHOD int SQLITE_TCLAPI DbEvalNextCmd(
...
*/
static SQLITE_METHOD void DbHookCmd(
  Tcl_Interp *interp,             /* Tcl interpreter */
  SqliteDb *pDb,                  /* Database handle */
...
*/
static SQLITE_METHOD int SQLITE_TCLAPI DbObjCmd(
  void *cd,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *const*objv
...
**
*/
static SQLITE_METHOD int SQLITE_TCLAPI DbMain(
  void *cd,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *const*objv
...
*/
EXTERN SQLITE_METHOD int Sqlite3_Init(Tcl_Interp *interp){
...
EXTERN SQLITE_METHOD int Tclsqlite3_Init(Tcl_Interp *interp){ return Sqlite3_Init(interp); }
EXTERN SQLITE_METHOD int Sqlite3_Unload(Tcl_Interp *interp, int flags){ return TCL_OK; }
EXTERN SQLITE_METHOD int Tclsqlite3_Unload(Tcl_Interp *interp, int flags){ return TCL_OK; }

/* Because it accesses the file-system and uses persistent state, SQLite
** is not considered appropriate for safe interpreters.  Hence, we cause
** the _SafeInit() interfaces return TCL_ERROR.
*/
EXTERN SQLITE_METHOD int Sqlite3_SafeInit(Tcl_Interp *interp){ return TCL_ERROR; }
EXTERN SQLITE_METHOD int Sqlite3_SafeUnload(Tcl_Interp *interp, int flags){return TCL_ERROR;}
...
*/
static SQLITE_METHOD const char *tclsh_main_loop(void){
...
SQLITE_METHOD int SQLITE_CDECL TCLSH_MAIN(int argc, char **argv){
  Tcl_Interp *interp;
  int i;
