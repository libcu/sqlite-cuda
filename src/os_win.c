...
#ifdef __cplusplus
#define ShmRegion winShmNode::ShmRegion
#endif /* __cplusplus */
...
static int winOpenSharedMemory(winFile *pDbFd){
  ...
  */
  p = (struct winShm *)sqlite3MallocZero( sizeof(*p) );
  if( p==0 ) return SQLITE_IOERR_NOMEM_BKPT;
  nName = sqlite3Strlen30(pDbFd->zPath);
  pNew = (struct winShmNode *)sqlite3MallocZero( sizeof(*pShmNode) + nName + 17 );
  if( pNew==0 ){
    sqlite3_free(p);
    return SQLITE_IOERR_NOMEM_BKPT;
  }
...
static int winGetTempname(sqlite3_vfs *pVfs, char **pzBuf){
  ...
  */
  ...
  zBuf = (char *)sqlite3MallocZero( nBuf );
  if( !zBuf ){
  ...
  */
  ...
  if( sqlite3_temp_directory ){
    ...
    char *zMulti;
    LPWSTR zWidePath = (LPWSTR)sqlite3MallocZero( nMax*sizeof(WCHAR) );
    if( !zWidePath ){
      sqlite3_free(zBuf);
      OSTRACE(("TEMP-FILENAME rc=SQLITE_IOERR_NOMEM\n"));
      return SQLITE_IOERR_NOMEM_BKPT;
    ...
    char *zUtf8;
    char *zMbcsPath = (char *)sqlite3MallocZero( nMax );
    if( !zMbcsPath ){
      sqlite3_free(zBuf);
      OSTRACE(("TEMP-FILENAME rc=SQLITE_IOERR_NOMEM\n"));
      return SQLITE_IOERR_NOMEM_BKPT;
...
static int winDelete(
  ...
  if( osIsNT() ){
    do {
...
#else
      attr = osGetFileAttributesW((LPCWSTR)zConverted);
#endif
      ...
      if ( osDeleteFileW((LPCWSTR)zConverted) ){
    ...
    do {
      attr = osGetFileAttributesA((LPCSTR)zConverted);
      if ( attr==INVALID_FILE_ATTRIBUTES ){
        lastErrno = osGetLastError();
        if( lastErrno==ERROR_FILE_NOT_FOUND
         || lastErrno==ERROR_PATH_NOT_FOUND ){
      ...
      if ( osDeleteFileA((LPCSTR)zConverted) ){
        rc = SQLITE_OK; /* Deleted OK. */
        break;
      }
      if ( !winRetryIoerr(&cnt, &lastErrno) ){
...
static int winFullPathname(
  ...
  */
  ...
  if( osIsNT() ){
    ...
    nByte += 3;
    zTemp = (LPWSTR)sqlite3MallocZero( nByte*sizeof(zTemp[0]) );
    if( zTemp==0 ){
      sqlite3_free(zConverted);
      return SQLITE_IOERR_NOMEM_BKPT;
    }
    ...
    nByte += 3;
    zTemp = (char *)sqlite3MallocZero( nByte*sizeof(zTemp[0]) );
    if( zTemp==0 ){
      sqlite3_free(zConverted);
      return SQLITE_IOERR_NOMEM_BKPT;
    }
...
static void (*winDlSym(sqlite3_vfs *pVfs,void *pH,const char *zSym))(void){
  FARPROC proc;
  UNUSED_PARAMETER(pVfs);
  proc = osGetProcAddressA((HMODULE)pH, zSym);
